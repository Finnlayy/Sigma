...
def test_closed_pnl_by_strategy_matches_python_group(tmp_path):
    ...
    store = DuckDBStore(str(tmp_path / "pnl-by-sid.duckdb"))
    store.upsert_trade({
    ...
    agg = store.closed_pnl_by_strategy()
    assert agg["s1"]["total_trades"] == 2
    assert agg["s1"]["winning_trades"] == 1
    assert agg["s1"]["realized_pnl"] == 6.0
    assert agg["s1"]["volume"] == 180.0
    assert agg["s2"]["total_trades"] == 1
    assert agg["s2"]["realized_pnl"] == 2.5
    assert "s1" in agg and "s2" in agg
