...
def _paper_balances(
    ...
    Pass *closed_trades* to reuse an in-request scan, or *paper_pnl* when the
    caller already ran closed_pnl_stats() (GET /api/logs). Otherwise SUM in SQL.
    """
    ...
    if closed_trades is not None:
        ...
        pnl = paper_pnl if paper_pnl is not None else state.store.sum_closed_pnl("paper")
        balances["USD"] = balances.get("USD", 0.0) + pnl
    return balances
...
async def logs():
    st = state
    strategies = st.store.list_strategies()
    # Order tape only needs the last 80 fills. Metrics / balances / strategyPnL
    # use DuckDB SUM/COUNT/GROUP BY so a 5–8s poll no longer materializes
    # 10k trade dicts. Bench @ 8k closed rows (same store as the prior 4×-scan
    # fix): trades(10000)+Python scan ~35.7 ms vs closed_pnl_stats +
    # closed_pnl_by_strategy + trades(80) ~4.1 ms (~9×, ~32 ms saved / poll).
...
def _build_metrics(
    ...
    closed_trades: Optional[List[Dict[str, Any]]] = None,
    paper_stats: Optional[Dict[str, float]] = None,
) -> Dict[str, Any]:
    from app.core.exchange_clock import get_exchange_clock
    ...
    if closed_trades is not None:
        ...
        paper_closed = [t for t in closed_trades if (t.get("execution_mode") or "paper") == "paper"]
        total_pnl = sum(float(t.get("net_pnl_usd") or 0.0) for t in paper_closed)
        total_trades = len(paper_closed)
    else:
        # SQL SUM+COUNT — /api/logs used to pull 10k rows just for these two numbers.
        stats = paper_stats if paper_stats is not None else st.store.closed_pnl_stats("paper")
        balances = _paper_balances(st, paper_pnl=stats["pnl"])
        total_pnl = float(stats["pnl"])
        total_trades = int(stats["count"])
...
def _strategy_pnl(
    ...
    if strategies is None:
        strategies = st.store.list_strategies()
    # Prefer SQL GROUP BY (closed_pnl_by_strategy). The Python scan path is
    # kept only when a caller already materialized rows for another reason.
    by_sid: Dict[str, Dict[str, float]] = {}
    if closed_trades is not None:
        for t in closed_trades:
            sid = str(t.get("strategy_id") or "")
            rec = by_sid.setdefault(sid, {"realized": 0.0, "wins": 0, "trades": 0, "volume": 0.0})
        ...
        for rec in st.store.closed_pnl_by_strategy():
            by_sid[str(rec.get("strategy_id") or "")] = rec
    open_positions = st.paper.all_positions()
    ...
    for s in strategies:
        rec = by_sid.get(s["id"], {})
        realized = float(rec.get("realized") or 0.0)
