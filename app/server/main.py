...
async def logs():
    st = state
    strategies = st.store.list_strategies()
    # Orders only need the 80 newest closed rows. Metrics / balances /
    # strategyPnL used to SELECT * LIMIT 10000 on every 5–8s poll
    # (bench @ 8k rows: ~28 ms). SQL COUNT/SUM/GROUP BY + 80-row fetch
    # is ~2–4 ms (~7–13×) and avoids ~10k Python dicts per poll.
...
def _build_metrics(
    ...
    if closed_trades is not None:
        ...
        # Bolt: COUNT+SUM in DuckDB (~0.8 ms @ 8k) instead of SELECT * LIMIT 10000.
        balances = _paper_balances(st)
...
def _strategy_pnl(
    ...
    if strategies is None:
        strategies = st.store.list_strategies()
    # Bolt: DuckDB GROUP BY (~1.4 ms @ 8k) instead of SELECT * LIMIT 5000 (~28 ms).
    # Injected closed_trades keeps the Python path for callers that already
    # hold a snapshot (tests / one-shot scans).
    if closed_trades is not None:
        by_sid: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
        for t in closed_trades:
            by_sid[str(t.get("strategy_id") or "")].append(t)
        aggs: Optional[Dict[str, Dict[str, Any]]] = None
    else:
        by_sid = {}
