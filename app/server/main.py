...
async def logs():
    st = state
    strategies = st.store.list_strategies()
    # Orders strip only needs the 80 newest fills. Metrics / balances /
    # strategyPnL are DuckDB aggregates — used to SELECT * LIMIT 10000
    # every 5–8s poll. Bench @ 8k rows / 20 strategies: scan+Python group
    # ~48 ms → aggregates + 80-row scan ~4.3 ms (~11×, ~44 ms / poll).
...
def _build_metrics(
    ...
    if closed_trades is not None:
        ...
        # SQL COUNT+SUM — do not pull 10k rows just for two numbers.
        balances = _paper_balances(st)
    ...
    if closed_trades is not None:
        ...
        for sid, mine in by_sid.items():
            agg[sid] = {
                ...
                "n": float(len(mine)),
                "wins": float(sum(1 for t in mine if float(t.get("net_pnl_usd") or 0) > 0)),
                "volume": sum(float(t.get("notional_usd") or 0.0) for t in mine),
            }
    else:
        # DuckDB GROUP BY — all closed trades, not a 5k-row Python group.
        agg = st.store.closed_pnl_by_strategy()
    out = []
    for s in strategies:
        stats = agg.get(s["id"], {"realized": 0.0, "n": 0.0, "wins": 0.0, "volume": 0.0})
        realized = float(stats["realized"])
        n = int(stats["n"])
        wins = int(stats["wins"])
        unrealized = 0.0
        ...
        out.append({
            ...
            "volumeTradedUSD": round(float(stats["volume"]), 2),
