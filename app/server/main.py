...
async def logs():
    st = state
    strategies = st.store.list_strategies()
    # Metrics / balances / strategyPnL are SQL aggregates (SUM/COUNT/GROUP BY).
    # Sharing one trades() scan still materialized ~10k full rows every 5–8s
    # poll (~35 ms @ 8k). Aggregates are ~1 ms; only the order strip needs rows.
    # Bench @ 4k closed rows: 1 scan ~14 ms → aggregates + 80-row strip ~3.5 ms (~4×).
    # At 8k the shared scan was ~35 ms; aggregates stay ~1–2 ms.
...
def _build_metrics(
    ...
    # Default (poll) path: DuckDB SUM/COUNT. Pass closed_trades only when a
    # caller already has the rows — do not re-scan 10k trades for two numbers.
    if closed_trades is not None:
        ...
        total_trades = paper_stats["count"]
...
def _strategy_pnl(
    ...
    if strategies is None:
        strategies = st.store.list_strategies()
    # Default: one SQL GROUP BY. Pass closed_trades only when the caller
    # already materialized rows — the 5–8s poll must not scan 5k trades.
    if closed_trades is not None:
        by_sid: Dict[str, Dict[str, Any]] = {}
        for t in closed_trades:
            rec = by_sid.setdefault(str(t.get("strategy_id") or ""), {
                "realized": 0.0, "trades": 0, "wins": 0, "volume": 0.0,
            })
        ...
        by_sid = st.store.strategy_closed_stats()
    open_positions = st.paper.all_positions()
    ...
    out = []
    for s in strategies:
        rec = by_sid.get(s["id"], {"realized": 0.0, "trades": 0, "wins": 0, "volume": 0.0})
        ...
        out.append({
            ...
            "volumeTradedUSD": round(float(rec["volume"]), 2),
