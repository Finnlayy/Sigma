...
async def logs():
    st = state
    strategies = st.store.list_strategies()
    # Orders need only the latest 80 closed rows. Metrics / balances / strategyPnL
    # used to share a 10k-row DuckDB scan (~34 ms @ 8k). SQL COUNT/SUM/GROUP BY
    # + LIMIT 80 is ~4.8 ms (~7×, ~29 ms saved / poll) on the 5–8s poll path.
...
def _build_metrics(
    ...
    # Prefer SQL COUNT+SUM on the poll path. A shared 10k-row snapshot was
    # still ~34 ms @ 8k; these two numbers are ~0.8 ms in DuckDB.
    if closed_trades is not None:
        ...
        stats = st.store.closed_paper_stats()
        total_pnl = stats["pnl"]
...
def _strategy_pnl(
    ...
    if strategies is None:
        strategies = st.store.list_strategies()
    # SQL GROUP BY on the poll path (~1.4 ms @ 8k). Keep the Python snapshot
    # path for callers that already materialized rows.
    if closed_trades is not None:
        ...
        aggs = st.store.strategy_closed_aggregates()
    open_positions = st.paper.all_positions()
    pos_by_sid: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
    for p in open_positions:
        pos_by_sid[str(p.get("strategy_id") or "")].append(p)
    _empty = {"n": 0, "realized": 0.0, "wins": 0, "volume": 0.0}
    out = []
    for s in strategies:
        a = aggs.get(s["id"], _empty)
        n = int(a["n"])
        realized = float(a["realized"])
        wins = int(a["wins"])
