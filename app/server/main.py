...
async def logs():
    st = state
    strategies = st.store.list_strategies()
    # Orders strip only (80 recent closed rows). Metrics / balances / strategyPnL
    # used to scan 10k full rows every 5–8s poll just to COUNT/SUM/GROUP BY.
    # SQL aggregates: ~5 ms vs ~41 ms @ 8k rows (~8×, ~36 ms saved / poll).
...
def _build_metrics(
    ...
    if closed_trades is not None:
        ...
        # SQL COUNT+SUM — do not pull 10k rows just to count/sum paper PnL.
        balances = _paper_balances(st)
...
def _strategy_pnl(
    ...
    if closed_trades is not None:
        ...
        # One GROUP BY instead of materializing 5k trade rows per poll.
        stats = st.store.strategy_closed_aggregates()
    open_positions = st.paper.all_positions()
    ...
    for s in strategies:
        a = stats.get(s["id"], {})
        realized = float(a.get("realized") or 0.0)
