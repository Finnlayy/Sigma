...
def _paper_balances(
    ...
    Pass *closed_trades* to reuse an in-request scan, or *paper_pnl* from
    ``closed_pnl_stats`` so GET /api/logs does not re-SUM. GET /api/logs used
    to call trades() four times per 5–8s poll (~40k row materializations).
    """
...
async def logs():
    st = state
    strategies = st.store.list_strategies()
    # Orders tape only needs the latest fills. Metrics / balances / strategyPnL
    # used to materialize up to 10k trade rows per 5–8s poll (~29 ms @ 8k).
    # SQL COUNT+SUM+GROUP BY + limit=80 is ~4 ms (~7×, ~25 ms/poll); all-time
    # stats, not a last-N snapshot.
...
def _build_metrics(
    ...
    if closed_trades is not None:
        ...
        # SQL COUNT+SUM — do not pull 10k rows just for two scalars.
        stats = paper_stats if paper_stats is not None else st.store.closed_pnl_stats("paper")
        balances = _paper_balances(st, paper_pnl=float(stats["pnl"]))
...
def _strategy_pnl(
    ...
    if strategies is None:
        strategies = st.store.list_strategies()
    # Prefer SQL GROUP BY (all-time, ~1 ms) over materializing last-N rows.
    # closed_trades remains a test / caller override.
    ...
    if closed_trades is not None:
        ...
        agg = st.store.strategy_closed_aggregates()
    open_positions = st.paper.all_positions()
    ...
    for s in strategies:
        if closed_trades is not None:
            ...
            wins = int(a.get("winning_trades") or 0)
