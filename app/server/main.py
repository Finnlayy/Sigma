...
async def logs():
    st = state
    strategies = st.store.list_strategies()
    # Orders only need ~80 recent fills. Metrics / balances / strategyPnL use
    # SQL aggregates (closed_pnl_stats + closed_stats_by_strategy) instead of
    # materializing up to 10k trade dicts on every 5–8s Overview poll.
    # Bench @ 8k rows: trades() scan ~31 ms → SUM/COUNT ~0.7 ms + GROUP BY
    # ~1.2 ms + LIMIT 80 ~1.8 ms (~5 ms, ~6×).
...
def _build_metrics(
    ...
    if closed_trades is not None:
        ...
        # SQL SUM+COUNT — avoid SELECT * LIMIT 10000 on the 5–8s poll path.
        balances = _paper_balances(st)
        paper_stats = st.store.closed_pnl_stats("paper")
        total_pnl = paper_stats["pnl"]
        total_trades = int(paper_stats["n"])
...
def _strategy_pnl(
    ...
    if strategies is None:
        strategies = st.store.list_strategies()
    # Prefer SQL GROUP BY (no 5k-row materialize). Injected closed_trades
    # keeps the Python path for callers that already hold a snapshot.
    if closed_trades is not None:
        by_sid: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
        for t in closed_trades:
            by_sid[str(t.get("strategy_id") or "")].append(t)
        agg: Dict[str, Dict[str, Any]] = {}
        for sid, mine in by_sid.items():
        ...
        agg = {r["strategy_id"]: r for r in st.store.closed_stats_by_strategy()}
    open_positions = st.paper.all_positions()
    pos_by_sid: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
    for p in open_positions:
        pos_by_sid[str(p.get("strategy_id") or "")].append(p)
    out = []
    empty = {"total_trades": 0, "winning_trades": 0, "realized": 0.0, "volume": 0.0}
    for s in strategies:
        rec = agg.get(s["id"], empty)
        realized = float(rec["realized"])
        wins = int(rec["winning_trades"])
        n = int(rec["total_trades"])
        unrealized = 0.0
