...
async def logs():
    st = state
    strategies = st.store.list_strategies()
    # Order tape only needs ~80 recent fills. Metrics / balances / strategyPnL
    # used to materialize 10k full trade dicts every 5–8s poll (~34 ms @ 8k).
    # SQL COUNT+SUM (~0.8 ms) + GROUP BY (~1.3 ms) + 80-row tape (~1.9 ms)
    # ≈ 4 ms/poll (~8×, ~30 ms saved). See closed_mode_stats.
...
def _build_metrics(
    ...
    if closed_trades is not None:
        ...
        # SQL COUNT+SUM — do not pull 10k rows on the 5–8s poll path.
        balances = _paper_balances(st)
        paper_stats = st.store.closed_mode_stats("paper")
...
def _strategy_pnl(
    ...
    if strategies is None:
        strategies = st.store.list_strategies()
    # Prefer SQL GROUP BY (all closed rows). Passing closed_trades keeps the
    # in-request Python path for callers that already scanned the table.
    if closed_trades is None:
        by_sid_stats = st.store.strategy_closed_stats()
    else:
        by_sid_stats = {}
        for t in closed_trades:
            rec = by_sid_stats.setdefault(
                str(t.get("strategy_id") or ""),
                {"realized": 0.0, "total_trades": 0, "wins": 0, "volume": 0.0},
            )
    ...
    out = []
    for s in strategies:
        agg = by_sid_stats.get(
            s["id"], {"realized": 0.0, "total_trades": 0, "wins": 0, "volume": 0.0}
        )
        realized = float(agg["realized"])
        wins = int(agg["wins"])
        n = int(agg["total_trades"])
