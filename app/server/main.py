...
async def logs():
    st = state
    strategies = st.store.list_strategies()
    # Order tape only needs the newest fills. Metrics / balances / strategy PnL
    # use DuckDB COUNT/SUM/GROUP BY — used to SELECT * LIMIT 10000 every 5–8s
    # poll (then re-scan that list in Python). Bench @ 8k closed rows:
    # 10k-row materialization ~38 ms → aggregates + 80-row tape ~6 ms
    # (~6×, ~32 ms saved / poll).
...
def _build_metrics(
    ...
    if closed_trades is not None:
        ...
        # Avoids pulling 10k full trade rows on the 5–8s dashboard poll.
        balances = _paper_balances(st)
        stats = st.store.closed_trade_stats("paper")
        total_pnl = stats["pnl"]
        total_trades = int(stats["n"])
...
def _strategy_pnl(
    ...
    if strategies is None:
        strategies = st.store.list_strategies()
    # Prefer SQL GROUP BY over materializing 5k row dicts (dashboard poll path).
    if closed_trades is None:
        sql_stats = st.store.strategy_closed_stats(limit=5000)
        by_sid: Optional[Dict[str, List[Dict[str, Any]]]] = None
    else:
        sql_stats = None
