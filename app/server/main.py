...
async def logs():
    st = state
    strategies = st.store.list_strategies()
    # Metrics / balances / strategyPnL are SQL aggregates (COUNT+SUM / GROUP BY).
    # Only the last 80 closed rows are materialized for the orders strip.
    # Was: trades(status="closed", limit=10000) every 5–8s poll — ~33 ms @ 8k
    # rows. Aggregates + 80-row orders fetch ~4 ms (~8×).
...
def _build_metrics(
    ...
    if closed_trades is not None:
        ...
        # Avoid the 10k-row trades() fallback — COUNT+SUM is enough for the
        # two dashboard numbers (totalTrades, profitLossPercentage).
...
def _strategy_pnl(
    ...
    if strategies is None:
        strategies = st.store.list_strategies()
    # Prefer a GROUP BY over materializing 5k trade rows for four numbers.
    if closed_trades is not None:
        by_sid: Dict[str, Dict[str, Any]] = {}
        for t in closed_trades:
            rec = by_sid.setdefault(
                str(t.get("strategy_id") or ""),
                {"realized": 0.0, "wins": 0, "trades": 0, "volume": 0.0},
            )
        ...
        by_sid = st.store.strategy_closed_aggregates()
    open_positions = st.paper.all_positions()
    ...
    out = []
    for s in strategies:
        rec = by_sid.get(s["id"], {"realized": 0.0, "wins": 0, "trades": 0, "volume": 0.0})
