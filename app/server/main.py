...
def _paper_balances(
    ...
    Pass *closed_trades* to reuse an in-request scan, or *paper_pnl* to reuse a
    SQL SUM already computed (closed_pnl_summary). GET /api/logs used to
    ...
    elif paper_pnl is not None:
        balances["USD"] = balances.get("USD", 0.0) + float(paper_pnl)
    else:
...
async def logs():
    st = state
    strategies = st.store.list_strategies()
    # Metrics / balances / strategyPnL are SQL aggregates (closed_pnl_summary +
    # closed_stats_by_strategy). Only the order tape needs recent rows —
    # pulling 10k full trade dicts on a 5–8s poll was ~31 ms @ 8k rows;
    # aggregates + LIMIT 80 is ~4 ms (~8×).
    paper_summary = st.store.closed_pnl_summary("paper")
    balances = _paper_balances(st, paper_pnl=paper_summary["pnl"])
    recent_closed = st.store.trades(status="closed", limit=80)
    open_positions = st.paper.all_positions()
    metrics = _build_metrics(st, strategies, paper_summary=paper_summary,
                             balances=balances)
...
def _build_metrics(
    ...
    if closed_trades is not None:
        # Legacy in-memory path (tests / callers that already have the scan).
        if balances is None:
            balances = _paper_balances(st, closed_trades)
        paper_closed = [t for t in closed_trades
                        if (t.get("execution_mode") or "paper") == "paper"]
        total_pnl = sum(float(t.get("net_pnl_usd") or 0.0) for t in paper_closed)
        total_trades = len(paper_closed)
    else:
        # SQL COUNT+SUM — do not pull 10k rows for two scalars.
        summary = paper_summary if paper_summary is not None else st.store.closed_pnl_summary("paper")
        total_pnl = float(summary["pnl"])
        total_trades = int(summary["count"])
        if balances is None:
            balances = _paper_balances(st, paper_pnl=total_pnl)
...
def _strategy_pnl(
    ...
    if closed_trades is not None:
        ...
        def _agg(sid: str) -> Dict[str, Any]:
            mine = by_sid.get(sid, [])
            return {
                "n": len(mine),
                "wins": sum(1 for t in mine if float(t.get("net_pnl_usd") or 0) > 0),
                "pnl": sum(float(t.get("net_pnl_usd") or 0.0) for t in mine),
                "volume": sum(float(t.get("notional_usd") or 0.0) for t in mine),
            }
    else:
        # SQL GROUP BY — same scalars as the Python scan, without 5k row dicts.
        stats = st.store.closed_stats_by_strategy()

        def _agg(sid: str) -> Dict[str, Any]:
            return stats.get(sid, {"n": 0, "wins": 0, "pnl": 0.0, "volume": 0.0})
    ...
    for s in strategies:
        ...
        realized = float(agg.get("pnl") or 0.0)
