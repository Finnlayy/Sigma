...
## 2026-08-31 - queue-matrices nested strategy filter
**Learning:** `GET /api/queue-matrices` (polled every 8s from Overview + Queue Matrix) filtered closed trades with `[t for t in q_trades if strategy_id == s.id]` per strategy — O(S×T) — and called `list_strategies()` twice (SELECT * including Pine `code`). Same anti-pattern as MetricsPanel `.find()` in `.map()`, on the backend poll path.
**Action:** On per-strategy poll endpoints, bucket trades once with `defaultdict(list)`. Call `list_strategies()` once per request; never SELECT strategy `code` twice just to split paper/live.
